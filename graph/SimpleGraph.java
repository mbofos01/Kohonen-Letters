...
public class SimpleGraph extends JPanel {
	...
	/**
	 * Creates a simple graphs where the step on each axis is 1 The maximum visible
	 * span of the x axis is set to 10. The maximum visible span of the y axis is
	 * set to 10.
	 */
	public SimpleGraph() {
		this(10, 10);
	...
	public SimpleGraph(double maxValueX, double maxValueY, double gridSpreadX, double gridSpreadY) {
		...
		popupMenu.add(new JMenuItem(new AbstractAction("Graph settings") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int option = JOptionPane.showConfirmDialog(SimpleGraph.this, settingsPanel, "Graph settings",
		...
		popupMenu.add(
				new JMenuItem(new CommonUIActions.SelectFile("Save as PNG", System.getProperty("user.home"), true) {
					@Override
					public void doWithSelectedDirectory(File selectedFile) {
						try {
							...
							save(selectedFile);
							JOptionPane.showMessageDialog(SimpleGraph.this, "Image successfully saved", "Information",
									JOptionPane.INFORMATION_MESSAGE);
						} catch (IOException e1) {
							JOptionPane.showMessageDialog(SimpleGraph.this, "An error occured during saving.", "Error",
		...
		// TODO: this could be changed to a state pattern to easily expand graph
		// functionality
	 ...
	 * This array needs to have a length of 2 or more otherwise adding the point
	 * will cause an exception. <br/>
	 * If the length is larger then 2 then only the 2 first elements are considered.
	 * <br/>
	...
	/**
	 * Adds the given points and shows them in the specified color. The graph <b>
	 * WILL NOT </b> automatically repaint.
	 */
	...
	protected void paintComponent(Graphics g) {
		...
		for (Pair<DoubleUnaryOperator, Color> functionPair : functions) {
			...
			for (double p = start; p <= end && i < xPoints.length; p += precision) {
				...
				if (y < -1 || y > height + 1)// -1 and +1 so the function line is not visibly connected at the
												// top/bottom of the graph
					continue;
		...
		for (IGraphShape shape : shapes) {
			shape.draw(g, x -> calculateX(yCoordinateWidth, x / maxValueX),
					y -> calculateY(yCoordinateStartHeight, y / maxValueY));
		}
	...
	// =============================================================================================================
	/**
	 * This interface needs to be implemented by a shape which can be plotted on the
	 * graph. <br/>
	 * It makes no restrictions on the type of shape only requesting that the draw
	 * method is implemented. <br/>
	 */
	public static interface IGraphShape {
		/**
		 * 
		 * @param g                      - graphics object used for painting the object
		 * @param xValueToScreenPosition - function which translates the x coordinate
		 *                               value to the position which needs to be given
		 *                               to the graphics object
		 * @param yValueToScreenPosition - function which translates the y coordinate
		 *                               value to the position which needs to be given
		 *                               to the graphics object
		 */
